...
describe('PhotosFavoritesComponent', () => {
  ...
  let captcha;
  UnitTestingModule.setUpTestBed(PhotosFavoritesComponent);
  ...
  it('check login response', (done) => {
    ...
    service1.userSignIn(data).subscribe((resp: any) => {
      ...
      localStorage.setItem('parentId', resp.parentId);
      expect(resp.responseCode).toBe(200);
      done();
    })
  });

  it('check show all favorite photos response', (done) => {
    let data = {
      asc: false,
      pageNb: 1,
      parentId: 0,
    ...
    service2.allFavotitesPhoto(data).subscribe((resp: any) => {
